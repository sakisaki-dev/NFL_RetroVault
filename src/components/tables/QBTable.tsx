...
const QBTable = ({ players }: QBTableProps) => {
          ...
          <tbody>
            {players.map((player) => {
              ...
              return (
                <tr 
                  key={player.name} 
                  className="hover:bg-secondary/20 transition-colors relative"
                  style={teamColors ? { 
                    borderLeft: `3px solid hsl(${teamColors.primary})`,
                  } : undefined}
                >
                  ...
                  <td><StatCell value={player.mvp} isLeader={isLeader(player.name, 'mvp')} /></td>
